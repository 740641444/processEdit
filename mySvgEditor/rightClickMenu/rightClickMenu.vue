<template>
  <div class="rightClickMenuBox">
      <div class="menuItem" v-for="(item,index) in menuList" :key="index" @click="menuItemFn(item)">
        ...
        <!-- <img :src="require('../../NBProcessEditor/svg/' + item.icon)" > -->
        <img :src="require('../../svg/GZL-节点信息.svg')" >
        <!-- {{item.icon}} -->
            ...
            menuType: [
                {
                    ...
                    label: "删除",
                    icon: "GZL-删除.svg",
                    power: ["node:click","edge:contextmenu","node:contextmenu"],
                    nodePower: ["nodeNormal","nodeRouter","nodeLogicJudge","nodeMerge","nodeSubFlow","edge"]
                },{
                    key: "nodeData",
                    label: "节点信息",
                    icon: "GZL-节点信息.svg",
